...
**作成日**: 2025-12-10  
**ステータス**: Phase1完了・Phase2進行中（遭遇/解答API + HUD統合 + 他プレイヤー描画 + 軽量化）  
**見積工数**: 約40時間（5-7日）

## 更新メモ (2025-12-11)
- three.js 軽量版＋R3F非依存に切替（SSR安全、フレーム安定）
- 3D/2DトグルとWebGL非対応フォールバックを追加
- FPSライク視点（マウスドラッグで視点、WASD/矢印で移動）
- キャンバスをメイン幅いっぱいに拡張、カーソルをgrab表示に統一
- Supabase Presenceで他プレイヤー表示（色付きキューブ＋名前ラベル、バトル中は赤発光、lerp補間）
- プレイヤー位置をZustandストアに集約しゾーン検知→自動遭遇（30%/4.5sクールダウン）
- 描画軽量化：ピクセル比上限1.5、shadow無効、机レイアウトを2x3に縮小
...
- HUD統合：`/virtual-classroom` で遭遇→回答→結果表示まで通過（バトルオーバーレイ＋報酬トースト、shake/confettiフラグをZustand管理）
- 3D表示：three.js軽量版Canvas3D（FPS視点、WASD、2Dフォールバック、他プレイヤー描画・lerp補間・バトル時赤発光、ピクセル比制限＆影オフ）
- プレイヤー座標をストアに集約しゾーン検知→30%確率で自動遭遇（クールダウン4.5s）
- 問題バンク：31体 × 5問 = 155問（プレースホルダで充足）＋AI生成フォールバック実装
- 未実装: 報酬アニメーション仕上げ、AI問題の品質向上＆実問題追加、3Dリッチ化の再検討（R3F復帰可否）
