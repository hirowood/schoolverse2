...
- HUD統合：`/virtual-classroom` で遭遇→回答→結果表示まで通過（バトルオーバーレイ＋報酬トースト）
- 簡易3D表示：キャンバスプレースホルダ導入（R3F置換予定）
- バトルUI: タイムリミット表示・日本語化、報酬トースト強化（勝利/失敗・XP/Coins表示）
- 問題バンク：31体 × 5問 = 155問（プレースホルダで充足）＋AI生成フォールバック実装
- 未実装: 本番3D/HUD統合（R3F）、報酬アニメーション強化、AI問題の品質向上＆実問題追加
