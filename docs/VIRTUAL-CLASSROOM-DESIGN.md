...
- デモUI：`/virtual-classroom/demo` と HUD `/virtual-classroom` で遭遇→回答→結果表示まで通過（バトルオーバーレイ＋報酬トースト）
- 問題バンク：31体 × 5問 = 155問（不足分をプレースホルダで充足済み）。さらなる実問題追加は今後。
- 未実装: AI問題生成呼び出し、3D/HUD統合の本番化（R3F置換）、報酬アニメーション強化、実問題リッチ化
