...
- ✅ バーチャル教室 Phase2: 軽量 Canvas3D + HUD 統合 + 他プレイヤー描画 + 軽量化（pixelRatio上限1.5、影オフ、机2x3）
- ✅ ゲーミフィケーション: XP/実績サービス・モックから実データ化、RewardToast 演出仕上げ（reduce-motion配慮・サウンドトグル・数値フォーマット）
- ✅ AI 問題品質向上（プロンプト正規化＆サニタイズ、各カテゴリ10問保証のシード拡充）
- ✅ スモーク自動テスト（主要5画面 + API）
- ✅ モバイル微調整（3D高さ可変、HUDボタン間隔）
...
| 優先 | タスク | 概要 | 目安 |
|---|---|---|---|
| 高 | AI問題品質 | プロンプト調整 + 10問/カテゴリ 追加 | ✅ |
| 中 | 3D軽量最適化 第2弾 | 影レンダリング停止、pixelRatio上限1.5、机2x3、FPS計測 | ✅ |
| 中 | スモークテスト | vitest/Playwright 主要5画面遷移 + API正常系 | ✅ |
| 低 | モバイル微調整 | 3D高さ可変、HUDボタン間隔 | ✅ |
