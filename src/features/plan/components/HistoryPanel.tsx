  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-slate-900">{PLAN_TEXT.selectedDate}: {selectedDate}</p>
        <button
          type="button"
          onClick={onAddClick}
          ...
          {tasks.length === 0 ? (
            ...
            tasks.map((t) => (
              <div key={t.id} className="rounded-md border border-slate-200 bg-white px-3 py-2">
                ...
                <p className="text-[11px] text-slate-500">{PLAN_TEXT.labelStatus}: {t.status}</p>
              </div>
            ))
          )}
        </div>
