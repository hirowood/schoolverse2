...
function getSharedPresenceClient(): SupabaseClient | null {
  if (typeof window === "undefined" || !SUPABASE_URL || !SUPABASE_ANON_KEY) return null;
  const g = globalThis as typeof globalThis & { __sbPresenceClient?: SupabaseClient };
  if (g.__sbPresenceClient) return g.__sbPresenceClient;
  g.__sbPresenceClient = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
    auth: {
    ...
    global: { headers: { "X-Client-Context": "presence" } },
  });
  return g.__sbPresenceClient;
}
...
export function usePresence(userId: string | null) {
  ...
  const supabase = useMemo<SupabaseClient | null>(() => {
    return getSharedPresenceClient();
  }, []);
