...
export function useUserChat() {
    ...
    async (roomId: string) => {
      ...
      try {
        if (process.env.NODE_ENV !== "production") {
          console.debug("[DEBUG] fetchMessages called for room:", roomId);
        }
        ...
        if (process.env.NODE_ENV !== "production") {
          console.debug("[DEBUG] fetchMessages got", data.messages?.length ?? 0, "messages");
        }
      } catch (e) {
        setError((e as Error).message);
      } finally {
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      recordActivity();
      if (process.env.NODE_ENV !== "production") {
        console.debug("[DEBUG] Sending message to room:", activeRoomId, "text length:", text.length);
      }
      ...
      if (process.env.NODE_ENV !== "production") {
        console.debug("[DEBUG] Send response status:", res.status);
        console.debug("[DEBUG] Calling fetchMessages after send room:", activeRoomId);
      }
