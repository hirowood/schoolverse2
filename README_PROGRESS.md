...
| **ユーザーチャット改善** | ✅ 完了 |
| **ダッシュボード改善** | ✅ 進行中 |
| **最終更新** | 2025-12-13（ダッシュボード: UI/UX再構築＋モバイル最適化、APIクライアント化・責務分離／既存: 学習チャット・AIコーチのモバイル入力改善） |
| **フェーズ** | MVP開発中 |
...
| ダッシュボード | 92% | ウィジェット統合、モバイル向けタップ領域/文字サイズ改善、UIプリミティブ統一、APIクライアント化 |
| AIコーチチャット | 92% | Claude API連携、プラン生成、タブUI/モバイル入力改善 |
| 学習チャット | 92% | セッション/メッセージ、SSEストリーム、レスポンシブUI（モバイルは履歴ドロワー） |
| ユーザーチャット | 100% | Presence・入力中・未読・モバイル対応 |
| タスク管理 | 90% | 階層構造、時間追跡、D&D対応 |
| 週次レポート | 90% | AI生成、Markdownエクスポート |
| バーチャル教室 | 96% | three.js軽量版＋2Dフォールバック、FPSカメラ＆WASD、他プレイヤー表示、2D補間同期、チャットパネル最適化 |

### 開発中（50-89%）

| 機能 | 進捗 | 説明 | 残作業 |
|------|:----:|------|--------|
| ノート機能 | 85% | キャンバス・OCR・AI分析 | UI改善、共通コンポーネント化 |
| マインドマップ | 85% | ReactFlow統合 | AI自動生成強化 |
| クエストシステム | 85% | AI生成デイリークエスト | バランス調整 |
| 実績システム | 88% | 定義・進捗追跡・カリキュラム連携 | UI強化 |
...
│   ├── features/               # 機能単位（api / ui / hooks 等）
│   ├── hooks/                  # 共通hooks（状態管理含む）
│   ├── lib/                    # 型・共通ユーティリティ
│   └── styles/                 # グローバルスタイル
...
| 日付 | バージョン | 変更内容 |
|------|-----------|----------|
| 2025-12-13 | v0.9.5 | ダッシュボード: UI/UX再構築（モバイル可読性・タップ領域）、責務分離（APIクライアント/ストア/表示）、重複クラス削減 |
| 2025-12-12 | v0.9.4 | 学習チャット: APIクライアント化＋SSE共通化、レスポンシブUI（モバイルは履歴ドロワー）、入力/バブル/一覧の可読性とタップ領域改善／AIコーチ: タブUIとモバイル入力改善 |
